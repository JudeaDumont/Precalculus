...
public class RightTriangle extends Triangle {
    ...
    public boolean pointInShape(Point point) {
        ...
        if (point.xCoordinate.compareTo(maxX) <= 0 && point.xCoordinate.compareTo(minX) >= 0 &&
            ...
            if (((point.xCoordinate.multiply(xRatio).add(point.yCoordinate.multiply(yRatio))).divide(new BigDecimal(1), new MathContext(EQUALITY_PRECISION))).compareTo(bound) <= 0) {
                inShape = true;
            }
        }
        return inShape;
