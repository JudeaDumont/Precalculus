...
public class TrianglePredicates {
    public static Triangle[] makeTrianglesFromLine(Line line) {
        return
                new Triangle[]{
                        Triangle.createInstance(new Point[]{new Point(line.getMidPoint()), new Point(line.point1),
                                new Point(line.getMidPoint().xCoordinate, line.point1.yCoordinate)}),
                        Triangle.createInstance(new Point[]{new Point(line.getMidPoint()), new Point(line.point2),
                                new Point(line.point2.xCoordinate, line.getMidPoint().yCoordinate)}),
                        Triangle.createInstance(new Point[]{new Point(line.getMidPoint()), new Point(line.point1),
                                new Point(line.point1.xCoordinate, line.getMidPoint().yCoordinate)}),
                        Triangle.createInstance(new Point[]{new Point(line.getMidPoint()), new Point(line.point2),
                                new Point(line.getMidPoint().xCoordinate, line.point2.yCoordinate)})};
    ...
    public static TriangleShapeType determineTriangleType(Line[] lines) {
        ...
        if ((lengths[0].equals(lengths[1]) || lengths[0].equals(lengths[2]) || lengths[1].equals(lengths[2])) &&
                ((lengths[0].pow(2).add(lengths[1].pow(2)))
                        .round(new MathContext((lengths[0].pow(2).add(lengths[1].pow(2))).
                                toBigInteger().toString().length())).stripTrailingZeros()
                        .toString().equals((lengths[2].pow(2).round(
                                new MathContext(lengths[2].pow(2).toBigInteger().toString().length()))
                    ...
                    new Point(distance.divide(new BigDecimal(2), new MathContext(SystemGlobal.CALC_PRECISION)),
                            new BigDecimal(Math.sqrt(distance.multiply(distance).subtract
                                    ((distance.multiply(distance).divide(new BigDecimal(4),
                                            new MathContext(SystemGlobal.CALC_PRECISION)))).doubleValue()))
                    )});
        } catch (Exception ex) {
            ex.printStackTrace();
