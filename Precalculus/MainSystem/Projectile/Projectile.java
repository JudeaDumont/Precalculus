...
import static MainSystem.SystemGlobal.SystemGlobal.XAXISRESISTANCE_CONSTANT;
...
public class Projectile extends Shape {
    ...
    public Projectile(Shape shape, BigDecimal accelerationDueToGravity, BigDecimal time, Point startingVelocity) {
        super(new Point[]{});
        trajectedObject = shape;
        initializeProjectile(accelerationDueToGravity, time, startingVelocity);
    }

    //Copy constructor is used to keep the original projectile before moving it across time.
    @SuppressWarnings("WeakerAccess")
    public Projectile(Projectile projectile) {
        ...
        trajectedObject = new Shape(projectile.trajectedObject, true) {
            ...
            @Override
            public void calcShapeArea() {
                trajectedObject.calcShapeArea();
            }

            @Override
            public Point[] centerShapePointsUpright() {
                return trajectedObject.centerShapePointsUpright();
            }

            @Override
            public boolean pointInShape(Point point) {
                return trajectedObject.pointInShape(point);
            }
        };
